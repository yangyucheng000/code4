...
void convolution_forward_cuda(at::Tensor in_feat, at::Tensor out_feat,
  ...
  neighbor_offset = neighbor_offset.cpu();
...
extern "C" int convolution_transpose_forward_ms(int nparam, void** params, int* ndims, int64_t** shapes,
  ...
  // Do the computation
  convolution_forward_cuda(in_feat, out_feat, kernel, neighbor_map, neighbor_offset, transpose);
  output.copy_(out_feat, false);
  return 0;
}
...
void convolution_backward_cuda(at::Tensor in_feat, at::Tensor grad_in_feat,
  ...
  int kernel_volume = kernel.size(0);
  neighbor_offset = neighbor_offset.contiguous().cpu();
  bool flag = false;
  int in_buffer_size;
  in_buffer_size =
      *std::max_element(neighbor_offset.data_ptr<int>(),
...
extern "C" int convolution_transpose_backward_ms(int nparam, void** params, int* ndims, int64_t** shapes,
  ...
  // Do the computation
  convolution_backward_cuda(in_feat, grad_in_feat, grad_out_feat, kernel,
                            grad_kernel, neighbor_map, neighbor_offset, transpose);

  output_grad_in_feat.copy_(grad_in_feat, false);
  output_grad_kernel.copy_(grad_kernel, false);
