...
extern "C" int devoxelize_forward_ms(int nparam, void** params, int* ndims, int64_t** shapes,
  ...
  // Do the computation
  at::Tensor tensor_out = devoxelize_forward_cuda(feat, indices, weight);
  output.copy_(tensor_out, false);
...
extern "C" int devoxelize_backward_ms(int nparam, void** params, int* ndims, int64_t** shapes,
  ...
  at::Tensor tensor_out = devoxelize_backward_cuda(top_grad, indices, weight, n);
  bottom_grad.copy_(tensor_out, false);
