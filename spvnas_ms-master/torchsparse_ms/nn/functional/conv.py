...
from torchsparse_ms.nn.cuda.convolution import SPConvolutionForward, SPConvolutionBackward
...
class ConvolutionFunction(nn.Cell):
    ...
    def construct(self,
        ...
        if get_context("device_target") == 'GPU':
            ...
            nbmaps = nbmaps.astype(ms.int32)
            nbsizes = nbsizes.astype(ms.int32)

            # mindspore to torch
            # input = torch.tensor(input.asnumpy(), dtype=torch.float32).cuda().contiguous()
            # output = torch.tensor(output.asnumpy(), dtype=torch.float32).cuda()
            # weight = torch.tensor(weight.asnumpy(), dtype=torch.float32).cuda().contiguous()
            # nbmaps = torch.tensor(nbmaps.asnumpy(), dtype=torch.int32).cuda().contiguous()
            # nbsizes = torch.tensor(nbsizes.asnumpy(), dtype=torch.int32).cuda().contiguous()
            ...
            #
            # output_conv3d = Tensor(output.detach().cpu().numpy(), dtype=ms.float32)
