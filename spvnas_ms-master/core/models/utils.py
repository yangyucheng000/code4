...
from torchsparse_ms.nn.utils import get_kernel_offsets
import numpy as np
...
def initial_voxelize(z, init_res, after_res):
    ...
    inserted_coords_sp = F.spvoxelize(ops.Floor()(new_float_coord), idx_query.astype(ms.int32),
                                   counts)
    ...
    # print(f"inserted_coords_sp :{inserted_coords_sp}")
    inserted_coords = ops.round(inserted_coords_sp).astype('int32')
    # print(f"inserted_coords:{inserted_coords}")
    # inserted_coords = inserted_coords.astype('int32')
    # print(f"inserted_coords_astype: {inserted_coords}")
...
def initial_voxelize_torch(z, init_res, after_res):
    new_float_coord = torch.cat(
        [(z.C[:, :3] * init_res) / after_res, z.C[:, -1].view(-1, 1)], 1)

    pc_hash = F_torch.sphash(torch.floor(new_float_coord).int())
    sparse_hash = torch.unique(pc_hash)
    idx_query = F_torch.sphashquery(pc_hash, sparse_hash)
    counts = F_torch.spcount(idx_query.int(), len(sparse_hash))

    inserted_coords_sp = F_torch.spvoxelize(torch.floor(new_float_coord), idx_query,
                                   counts)
    inserted_coords = torch.round(inserted_coords_sp).int()
    inserted_feat = F_torch.spvoxelize(z.F, idx_query, counts)
...
def point_to_voxel_torch(x, z):
    if z.additional_features is None or z.additional_features.get(
            'idx_query') is None or z.additional_features['idx_query'].get(
                x.s) is None:
        pc_hash = F_torch.sphash(
            torch.cat([
                torch.floor(z.C[:, :3] / x.s[0]).int() * x.s[0],
                z.C[:, -1].int().view(-1, 1)
            ], 1))
        sparse_hash = F_torch.sphash(x.C)
        idx_query = F_torch.sphashquery(pc_hash, sparse_hash)
        counts = F_torch.spcount(idx_query.int(), x.C.shape[0])
        z.additional_features['idx_query'][x.s] = idx_query
        z.additional_features['counts'][x.s] = counts
    else:
        idx_query = z.additional_features['idx_query'][x.s]
    ...
    inserted_feat = F_torch.spvoxelize(z.F, idx_query, counts)
    new_tensor = torchsparse.SparseTensor(inserted_feat, x.C, x.s)
    new_tensor.cmaps = x.cmaps
    new_tensor.kmaps = x.kmaps
...
def voxel_to_point(x, z, nearest=False):
    if z.idx_query is None or z.weights is None or z.idx_query.get(
        ...
        pc_hash = ms.Tensor(pc_hash.asnumpy(), dtype=pc_hash.dtype)
...
def voxel_to_point_torch(x, z, nearest=False):
    if z.idx_query is None or z.weights is None or z.idx_query.get(
            x.s) is None or z.weights.get(x.s) is None:
        off = get_kernel_offsets_torch(2, x.s, 1, device=z.F.device)
        old_hash = F_torch.sphash(
            torch.cat([
                torch.floor(z.C[:, :3] / x.s[0]).int() * x.s[0],
                z.C[:, -1].int().view(-1, 1)
            ], 1), off)
        pc_hash = F_torch.sphash(x.C.to(z.F.device))
        idx_query = F_torch.sphashquery(old_hash, pc_hash)
        weights = F_torch.calc_ti_weights(z.C, idx_query,
                                    scale=x.s[0]).transpose(0, 1).contiguous()
        idx_query = idx_query.transpose(0, 1).contiguous()
        if nearest:
            weights[:, 1:] = 0.
            idx_query[:, 1:] = -1
        new_feat = F_torch.spdevoxelize(x.F, idx_query, weights)
        new_tensor = torchsparse.PointTensor(new_feat,
                                 z.C,
                                 idx_query=z.idx_query,
                                 weights=z.weights)
        new_tensor.additional_features = z.additional_features
        ...
        new_feat = F_torch.spdevoxelize(x.F, z.idx_query.get(x.s), z.weights.get(x.s))
        new_tensor = torchsparse.PointTensor(new_feat,
                                 z.C,
                                 idx_query=z.idx_query,
                                 weights=z.weights)
