...
class TernaryLoss(Loss):
    def __init__(self, loss_weight, keys, patch_size=7):
        ...
        self.w = ms.tensor(self.w, dtype=ms.float32)
    ...
    def transform(self, tensor):
        ...
        patches = ops.conv2d(tensor_, self.w, padding=self.patch_size//2, bias=None)
        loc_diff = patches - tensor_
        # loc_diff_norm = loc_diff / torch.sqrt(0.81 + loc_diff ** 2)
        loc_diff_norm = loc_diff / ops.sqrt(0.81 + loc_diff ** 2)
    ...
    def valid_mask(self, tensor):
        ...
        inner = ops.ones((b, 1, h - 2 * padding, w - 2 * padding)).type_as(tensor)
        mask = ops.pad(inner, [padding] * 4)
        return mask
...
class GeometryLoss(Loss):
    def __init__(self, loss_weight, keys, patch_size=3):
        ...
        self.w = ops.tensor(self.w).float()
    ...
    def transform(self, tensor):
        ...
        loc_diff_norm = loc_diff_ / ops.sqrt(0.81 + loc_diff_ ** 2)
        return loc_diff_norm
...
class IFRFlowLoss(Loss):
    ...
    def resize(self, x, scale_factor):
        ...
        return scale_factor * ops.interpolate(x, scale_factor=scale_factor, mode="bilinear", align_corners=False)
    ...
    def get_robust_weight(self, flow_pred, flow_gt):
        ...
        robust_weight = ops.exp(-self.beta * epe)
